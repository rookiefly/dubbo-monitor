...
function drawChart(data) {
    var newMethod = data.method.replace(/\(|,|\)/ig, "_");
    if(newMethod.charAt(newMethod.length-1) == '_'){
        newMethod = newMethod.substr(0, newMethod.length-1);
    }
    $('#' + data.chartType + '_' + newMethod).highcharts({
        chart: {
            zoomType: 'x'
        }, title: {
